...
const double tos = 30.0;
const double thyst = 28.0;
int tos_data = (int)(tos / 0.5) << 7;
int thyst_data = (int)(thyst / 0.5) << 7;
const uint8_t osPin = 2;
...
uint8_t loopCnt = 0;
static _Bool gb_didProceedData = 0;
...
int main(void)
{
    ...
    while (1)
    {
        ...
        if(!I2C->IsBusy() && gb_didProceedData)
        {
            I2C->Read(0x48, dataRead, 2);
            gb_didProceedData = 0;
        }
        ...
        {
            er = I2C->ErrorGet();
            if (er == I2C_ERROR_NONE)
            {



                rawTempValue = (dataRead[0] << 8 | dataRead[1]);
                tempCelsius = (float)(rawTempValue >> 5) * 0.125;
